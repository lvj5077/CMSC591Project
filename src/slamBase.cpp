...
RESULT_OF_PNP estimateMotion( FRAME& frame1, FRAME& frame2, CAMERA_INTRINSIC_PARAMETERS& camera )
{
    ...
    if (pts_obj.size() <5 || pts_img.size()==0)
    {
        result.inliers = -1;
        return result;
    }
    ...
    // 求解pnp
    cout << pts_obj.size() << " matches"<<endl;
    cout << "pnp"<<endl;
    cv::solvePnPRansac( pts_obj, pts_img, cameraMatrix, cv::Mat(), rvec, tvec, false, 100, 100, .99, inliers );
    ...
    // 2.4 void solvePnPRansac(InputArray objectPoints, InputArray imagePoints, InputArray cameraMatrix, InputArray distCoeffs, OutputArray rvec, OutputArray tvec, bool useExtrinsicGuess=false, int iterationsCount=100, float reprojectionError=8.0, int minInliersCount=100, OutputArray inliers=noArray(), int flags=ITERATIVE )
    cout << "solvePnp"<<endl;
    result.rvec = rvec;
    result.tvec = tvec;
